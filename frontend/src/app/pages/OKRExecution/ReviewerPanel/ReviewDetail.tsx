...
export default function ReviewDetail() {
  ...
  const confirmReject = async () => {
    ...
    try {
      ...
      ToastService.success("Changes requested. Feedback has been sent to the employee.");
    ...
    return (
      <EmployeeLayout>
        <div className="py-20 flex flex-col items-center justify-center gap-4">
          ...
          <p className="text-sm text-slate-500 animate-pulse">Loading submission details...</p>
        </div>
      </EmployeeLayout>
    );
  }
    ...
    <EmployeeLayout>
      <div className="min-h-screen bg-slate-50/50 -mx-4 md:-mx-8 px-4 md:px-8 pb-32">
        <ExecutionShell
          ...
          icon={submission.plan_type === "MONTHLY" ? <MdDateRange className="text-2xl" /> : <MdEventNote className="text-2xl" />}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex flex-wrap items-center justify-between gap-6">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-full bg-slate-100 flex items-center justify-center overflow-hidden border border-slate-200">
                  {submission.avatar_url ? (
                    <img src={submission.avatar_url} alt={submission.employee_name} className="w-full h-full object-cover" />
                  ) : (
                    <MdPerson className="text-slate-400 text-2xl" />
                  )}
                </div>
                <div>
                  <h2 className="text-xl font-bold text-slate-800">{submission.employee_name}</h2>
                  <p className="text-sm text-slate-500">Submitted on {new Date(submission.submitted_at).toLocaleString()}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-8">
                ...
                <div className="text-center">
                  <p className="text-[10px] text-slate-400 uppercase tracking-widest font-bold mb-1">Items</p>
                  <span className="text-lg font-bold text-slate-800">{submission.item_count}</span>
                </div>
              </div>
            </div>
                    ...
                    <div className="p-6">
                      <div className="flex items-start justify-between gap-4 mb-4">
                        ...
                        <div className="shrink-0">
                          <Button 
                            variant={openComments[item.id] ? "primary" : "white"} 
                            size="sm" 
                            icon={MdMessage}
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        <div className="px-3 py-1.5 bg-slate-50 rounded-lg border border-slate-100 flex items-center gap-2">
                          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Associated KR:</span>
                          <span className="text-xs font-semibold text-slate-700">{item.parent_kr_title || item.kr_title || "General"}</span>
                        </div>
                        {item.weight_pct > 0 && (
                          <div className="px-3 py-1.5 bg-slate-50 rounded-lg border border-slate-100 flex items-center gap-2">
                            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Weight:</span>
                            <span className="text-xs font-bold text-primary">{item.weight_pct}%</span>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    {openComments[item.id] && (
                      <div className="bg-slate-50/50 border-t border-slate-100 p-6">
                        <CommentThread 
                          ...
                          itemId={item.id}
                          itemType={submission.plan_type === "MONTHLY" ? "MONTHLY_PLAN" : "WEEKLY_PLAN"}
                        />
                      </div>
                    )}
                  </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-md border-t border-slate-200 p-4 shadow-2xl z-50">
          <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
            <div className="hidden sm:block">
              <p className="text-sm font-semibold text-slate-800">Finalize Review</p>
              <p className="text-xs text-slate-500">Provide feedback or approve the execution plan.</p>
            </div>
          ...
          message={
            <div className="space-y-4">
              <p className="text-sm text-slate-600">
                Please provide feedback explaining why the plan needs changes. This will be sent to {submission?.employee_name}.
              </p>
              <textarea
