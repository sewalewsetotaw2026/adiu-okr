...
export interface EditMonthlyPlanModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function EditMonthlyPlanModal({
  isOpen,
  onClose,
  plan,
  onSaved,
}: EditMonthlyPlanModalProps) {
  ...
  const onSave = async () => {
    ...
    if (targetValue !== "") {
      ...
      if (tv > parentKrTarget) {
        errs.targetValue = `Target (${tv}) exceeds parent KR target (${parentKrTarget}${parentUnit ? ` ${parentUnit}` : ""}). Reduce the value.`;
      }
    ...
    >
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <div className="p-6 space-y-4">
          <div className="rounded-xl bg-slate-50 ring-1 ring-inset ring-slate-100 px-3 py-2 text-xs text-slate-600">
            <div>
