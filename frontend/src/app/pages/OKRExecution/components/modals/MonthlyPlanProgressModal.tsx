...
interface MonthlyPlanProgressModalProps {
  isOpen: boolean;
  onClose: () => void;
  plan: MonthlyPlan;
  onSave: (data: {
...
export default function MonthlyPlanProgressModal({
  isOpen,
  onClose,
  plan,
  onSave,
}: MonthlyPlanProgressModalProps) {
      ...
      <div className="space-y-5">
        ...
        <div className="p-4 rounded-2xl bg-linear-to-r from-primary/5 to-primary/10 border border-primary/10">
          ...
          <div className="flex items-center gap-3 text-xs text-slate-500">
            <span className="font-semibold">Month {plan.month_number}</span>
            <span className="w-1 h-1 rounded-full bg-slate-300" />
            <span>{plan.parent_key_result?.title || "Key Result"}</span>
          </div>
        </div>
