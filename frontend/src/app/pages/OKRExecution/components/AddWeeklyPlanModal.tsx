...
export default function AddWeeklyPlanModal({
  ...
  const [allocations, setAllocations] = useState<Record<string, AvailableWeight | null>>({});
  ...
  const selectedPlan = useMemo(() => monthlyPlans.find((p) => p.id === selectedMonthlyId) ?? null, [monthlyPlans, selectedMonthlyId]);
  ...
  useEffect(() => {
    ...
    Promise.all(krIds.map((id) => fetchMonthlyPlans(id).catch(() => [] as MonthlyPlan[])))
      .then((results) => {
        if (cancelled) return;
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    Promise.all(monthlyPlans.map(async (mp) => {
      try { return [mp.id, await fetchWeeklyAvailableWeight(mp.id)] as const; }
      catch { return [mp.id, null] as const; }
    })).then((entries) => {
  ...
  useEffect(() => {
    ...
    fetchManagerWeeklyPlans(String(selectedPlan.aligned_manager_plan_id), 0)
  ...
  const validate = (): boolean => {
    ...
    subtasks.forEach((st, i) => {
      ...
      if (st.title.length > TITLE_MAX) errs[`${pfx}title`] = `Max ${TITLE_MAX} chars.`;
      if (!st.weekNumber) errs[`${pfx}week`] = "Select a week.";
      const w = Number(st.weight);
      if (!Number.isFinite(w) || w <= 0) errs[`${pfx}weight`] = "Enter weight > 0.";
      ...
      if (selectedPlan && st.targetValue !== "") {
        ...
        if (tv > pT) errs[`${pfx}targetValue`] = `Exceeds monthly plan target (${pT}).`;
        if (st.startValue !== "" && Number(st.startValue) < pS) errs[`${pfx}startValue`] = `Below plan start (${pS}).`;
        if (st.startValue !== "" && Number(st.startValue) > tv) errs[`${pfx}startValue`] = "Start cannot exceed target.";
      }
    });
  ...
  const onSave = async () => {
    ...
    try {
      ...
      for (const st of subtasks) {
        ...
        created.push(await createWeeklyPlan(selectedMonthlyId, {
          ...
          aligned_manager_plan_id: st.alignedManagerPlanId ? Number(st.alignedManagerPlanId) : undefined,
        }));
      }
      onCreated(created);
      onClose();
  ...
  const canSave = canProceed && subtasks.every((st) => st.title.trim() && st.weekNumber !== "") &&
    ...
    <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm ${tc} ${isFullView ? "p-2" : "p-4"}`}
      ...
      <div className={`bg-white rounded-3xl shadow-2xl w-full flex flex-col overflow-hidden ${tc} ${isFullView ? "max-w-6xl h-[96vh]" : "max-w-2xl h-[92vh]"}`}>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          {isFullView ? (
            <FullViewLayout
              candidates={candidates} loading={loadingMonthly} allocations={allocations}
              selectedId={selectedMonthlyId} onSelectPlan={handleSelectPlan}
              selectedPlan={selectedPlan} remainingPct={remainingPct} allocatedPct={allocatedPct}
              ...
              preselectedWeekNumber={preselectedWeekNumber} selectedAW={selectedAW}
            ...
            <>
              {step === 1 && (
                <Step1
                  candidates={candidates} loading={loadingMonthly} allocations={allocations}
                  selectedId={selectedMonthlyId} onSelect={handleSelectPlan}
                />
              )}
              {step === 2 && (
                selectedPlan ? (
                  <ConfigurePanel
                    plan={selectedPlan} remainingPct={remainingPct}
...
function Step1({
  candidates, loading, allocations, selectedId, onSelect,
}: {
  candidates: MonthlyPlan[]; loading: boolean; allocations: Record<string, AvailableWeight | null>;
  selectedId: string | null; onSelect: (id: string) => void;
}) {
  if (loading) return <div className="text-center py-8 text-sm text-slate-500">Loading monthly plans…</div>;
  if (candidates.length === 0) return (
    <div className="text-center py-10 px-4">
      <div className="text-base font-semibold text-slate-700 mb-1">Nothing available</div>
      <p className="text-sm text-slate-500">No published monthly plans with remaining capacity. Publish a monthly plan first.</p>
    </div>
  );
  ...
  return (
    <div className="space-y-3">
      {candidates.map((mp) => {
        ...
        const progress = Math.max(0, Math.min(100, Number(mp.progress_pct ?? 0)));
        ...
        return (
          <div key={mp.id}>
            <button type="button" onClick={() => onSelect(mp.id)}
              ...
              <div className="flex items-start justify-between gap-3 mb-2">
                <div className="min-w-0">
                  <div className="text-[10px] font-black tracking-widest uppercase text-slate-400">Month {mp.month_number}</div>
                  <h4 className="text-sm font-bold text-slate-800 truncate">{mp.title}</h4>
                </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-1 text-xs text-slate-500">
                <span className="tabular-nums">{current} / {target}{unit ? ` ${unit}` : ""}</span>
                <span>
                  <span className="font-semibold text-slate-700 tabular-nums">{allocated}%</span> planned ·{" "}
                  <span className="font-semibold text-emerald-700 tabular-nums">{remaining}%</span> remaining
                </span>
              </div>
              {aw && aw.parent_target_value != null && aw.parent_target_value > 0 && (
                <div className="flex items-center justify-between text-[11px] text-slate-400 mt-1 pt-1 border-t border-slate-100">
                  <span><span className="tabular-nums font-medium text-slate-500">{aw.allocated_target_value ?? 0}</span> / <span className="tabular-nums">{aw.parent_target_value}</span>{unit ? ` ${unit}` : ''} allocated</span>
                  <span><span className="tabular-nums font-semibold text-emerald-600">{aw.remaining_target_value ?? 0}</span>{unit ? ` ${unit}` : ''} remaining</span>
                </div>
              )}
            </button>
          </div>
...
function ConfigurePanel({
  plan, remainingPct, allocatedPct, subtasks, onUpdateSubtask,
  onAddSubtask, onRemoveSubtask, canAddSubtask, totalSubtaskWeight, metrics, fieldErrors, error,
  preselectedWeekNumber, selectedAW, managerWeeklyPlans, loadingManagerWeekly,
  userRoleLevel,
}: {
  plan: MonthlyPlan; remainingPct: number; allocatedPct: number;
  ...
  totalSubtaskWeight: number; metrics: MetricDefinition[];
  fieldErrors: Record<string, string>; error: string | null; preselectedWeekNumber?: number;
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl bg-slate-50 ring-1 ring-inset ring-slate-100 px-4 py-3 text-xs flex flex-wrap gap-x-4 gap-y-1">
        <span><span className="font-black tracking-widest uppercase text-[10px] text-slate-400 mr-1">Plan:</span><span className="font-bold text-slate-800">M{plan.month_number} — {plan.title}</span></span>
      ...
      {hasTargetInfo && (
        <div className="rounded-xl bg-emerald-50/60 ring-1 ring-inset ring-emerald-100 px-4 py-2 text-xs flex items-center justify-between">
          <span className="text-emerald-700">Target capacity: <strong className="tabular-nums">{selectedAW!.allocated_target_value ?? 0}</strong> / <strong className="tabular-nums">{selectedAW!.parent_target_value}</strong>{unit ? ` ${unit}` : ''} allocated</span>
          <span className="font-bold text-emerald-800 tabular-nums">{selectedAW!.remaining_target_value ?? 0}{unit ? ` ${unit}` : ''} remaining</span>
        </div>
      )}
      ...
      <div className="space-y-3">
        {subtasks.map((st, i) => (
          <SubtaskCard key={st.id} index={i} subtask={st}
            ...
            metrics={metrics} fieldErrors={fieldErrors}
            plan={plan} preselectedWeekNumber={preselectedWeekNumber} selectedAW={selectedAW}
            managerWeeklyPlans={st.weekNumber ? managerWeeklyPlans.filter(mp => mp.week_number === st.weekNumber) : []}
            loadingManagerWeekly={loadingManagerWeekly}
            userRoleLevel={userRoleLevel}
          />
        ))}
...
function SubtaskCard({
  index, subtask, onUpdate, onRemove, canRemove, remainingPct,
  metrics, fieldErrors, plan, preselectedWeekNumber, selectedAW, managerWeeklyPlans, loadingManagerWeekly,
  userRoleLevel,
}: {
  index: number; subtask: SubtaskForm; onUpdate: (p: Partial<SubtaskForm>) => void; onRemove: () => void;
  canRemove: boolean; remainingPct: number;
  metrics: MetricDefinition[]; fieldErrors: Record<string, string>; plan: MonthlyPlan;
  preselectedWeekNumber?: number; selectedAW?: AvailableWeight | null;
  managerWeeklyPlans: WeeklyPlan[];
  loadingManagerWeekly: boolean;
  userRoleLevel?: string;
}) {
  ...
  useEffect(() => {
    if (preselectedWeekNumber && subtask.weekNumber === "") onUpdate({ weekNumber: preselectedWeekNumber });
  }, []);
  ...
  useEffect(() => {
    ...
    if (!u) return;
    const match = metrics.find((m) => (m.unit_of_measure ?? m.unit ?? "").toLowerCase() === u.toLowerCase());
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 space-y-3">
      <div className="flex items-center justify-between">
        <span className="text-sm font-bold text-slate-800">
          Subtask {index + 1}
          {weight > 0 && (
            <span className="ml-2 text-xs font-normal text-primary/70">({weight}% of plan)</span>
          )}
        </span>
        {canRemove && (
      ...
      <Field label="Week" required error={fieldErrors[`${pfx}week`]}>
        {(() => {
          ...
          const weekNumbers = [1, 2, 3, 4, 5].filter(w => availableWeeks.some(aw => aw.week_number === w && (aw.plan_id === null || subtask.weekNumber === w)));
          return (
            <select value={subtask.weekNumber}
              onChange={(e) => onUpdate({ weekNumber: e.target.value === "" ? "" : Number(e.target.value) })}
      ...
      <Field label="Align with Manager Weekly Plan" error={fieldErrors[`${pfx}alignment`]}>
        {loadingManagerWeekly ? (
          <div className="animate-pulse h-10 bg-slate-100 rounded-xl" />
        ) : !plan.aligned_manager_plan_id ? (
          <div className="rounded-xl bg-slate-50 border border-slate-200 p-3">
            <p className="text-[10px] text-slate-500">This monthly plan is not aligned with any manager plan, so no weekly alignment is possible.</p>
          </div>
          ...
          <div className="rounded-xl bg-slate-50 border border-slate-200 p-2 text-[10px] text-slate-500 italic">
            Alignment is managed at the quarterly objective level for Department Managers.
          </div>
        ) : (
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        {!hideValues && (
          <div className="grid grid-cols-2 gap-2">
            ...
            <Field label="Target Value" error={fieldErrors[`${pfx}targetValue`]}>
              <input type="number" value={subtask.targetValue} onChange={(e) => onUpdate({ targetValue: e.target.value })}
                placeholder={autoTarget ? "Auto: " + autoTarget.toFixed(2) : "Target"}
                className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm outline-none" />
            </Field>
          </div>
        )}
      ...
      <div className="pt-2 flex flex-col gap-2">
        {!selectedMetric?.value_based_progress && (
          <label className="flex items-center gap-3 cursor-pointer group">
            <input type="checkbox" checked={subtask.contributeToScore}
              onChange={(e) => onUpdate({ contributeToScore: e.target.checked })}
              className="w-4 h-4 rounded text-primary border-slate-300" />
            <span className="text-xs font-medium text-slate-700">Contribute to Score</span>
          </label>
        )}
        <label className="flex items-center gap-3 cursor-pointer group">
          <input type="checkbox" checked={subtask.contributeToValue}
            onChange={(e) => onUpdate({ contributeToValue: e.target.checked })}
            className="w-4 h-4 rounded text-primary border-slate-300" />
          <span className="text-xs font-medium text-slate-700">Contribute to Value</span>
        </label>
      </div>
    </div>
  );
...
function FullViewLayout({
  candidates, loading, allocations, selectedId, onSelectPlan, selectedPlan,
  remainingPct, allocatedPct,
  subtasks, onUpdateSubtask, onAddSubtask, onRemoveSubtask, canAddSubtask, totalSubtaskWeight,
  metrics, fieldErrors, error, preselectedWeekNumber, setStep, selectedAW,
  managerWeeklyPlans, loadingManagerWeekly,
  userRoleLevel,
}: {
  candidates: MonthlyPlan[]; loading: boolean; allocations: Record<string, AvailableWeight | null>;
  selectedId: string | null; onSelectPlan: (id: string) => void; selectedPlan: MonthlyPlan | null;
  remainingPct: number; allocatedPct: number;
  subtasks: SubtaskForm[]; onUpdateSubtask: (id: string, p: Partial<SubtaskForm>) => void;
  onAddSubtask: () => void; onRemoveSubtask: (id: string) => void; canAddSubtask: boolean;
  totalSubtaskWeight: number; metrics: MetricDefinition[];
  fieldErrors: Record<string, string>; error: string | null; preselectedWeekNumber?: number;
  setStep: (s: Step) => void; selectedAW: AvailableWeight | null;
  managerWeeklyPlans: WeeklyPlan[];
  loadingManagerWeekly: boolean;
  userRoleLevel?: string;
}) {
      ...
      <div className={`lg:col-span-2 flex flex-col gap-3 ${step1Done ? "opacity-90" : ""}`}>
        ...
        <div className="flex-1 bg-slate-50/50 rounded-2xl border border-slate-200 overflow-hidden">
          <div className="h-full overflow-y-auto p-3">
            <Step1
              candidates={candidates} loading={loading} allocations={allocations}
              selectedId={selectedId} onSelect={(id) => { onSelectPlan(id); setStep(2); }} />
          </div>
        </div>
      </div>
