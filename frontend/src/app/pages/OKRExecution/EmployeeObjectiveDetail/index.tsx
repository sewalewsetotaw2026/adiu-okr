...
import KRModal from "../../Admin/OKR/components/KRModal";

import { useParams, useNavigate, useLocation } from "react-router-dom";
import { routeConstants, getRoleNameById } from "../../../../utils/constants";
...
import { Status } from "../../Admin/OKR/components/StatusBadge";
import StatusBadge from "../../Admin/OKR/components/StatusBadge";
...
export default function EmployeeObjectiveDetailPage() {
  ...
  const fetchObjective = async () => {
    ...
    try {
      ...
      // Fetch feedback comments if in draft status (possibly reverted)
      let allComments: any[] = [];
      if (data?.status_code === "draft" && !isAdminView) {
        try {
          const commentRoute = data.submission_id
            ? apiRoutes.okr.submissionComments(data.submission_id)
            : `${apiRoutes.okr.entityComments}?entity_type=EMPLOYEE_OBJECTIVE&entity_id=${objectiveId}`;

          const commentRes = await makeCall({
            method: "GET",
            route: commentRoute,
            isSecureRoute: true,
          });
      ...
      <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 pb-16 space-y-8 pt-2">
          ...
          <nav className="flex flex-wrap items-center gap-3 text-sm pt-4">
            ...
            <MdChevronRight className="text-gray-300 shrink-0" />
            <span className="text-gray-800 font-medium truncate max-w-[12rem] sm:max-w-none px-2">
              {objective.title}
            </span>
          </nav>
          ...
          {/* FEEDBACK SECTION */}
          {comments.length > 0 && (
            <div className="rounded-3xl border-2 border-amber-100 bg-amber-50/50 p-6 shadow-sm">
