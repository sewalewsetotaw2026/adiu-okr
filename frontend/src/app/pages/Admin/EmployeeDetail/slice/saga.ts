  ...
  try {
    ...
    if (!section || section === "contact") {
      ...
      if (p.emergencyContacts) {
        ...
        promises.push(employeeService.updateEmergencyContacts(id, p.emergencyContacts));
      }
    }
    ...
    if ((!section || section === "education") && body.education) {
      ...
      for (const edu of body.education) {
        ...
        if (edu.document_urls && Array.isArray(edu.document_urls)) {
          ...
          processedDocUrls.push(edu.documentUrl);
        }
        ...
        if (edu.document instanceof File) {
          try {
            ...
            throw new Error(`Failed to upload education document: ${e.message}`);
          }
        }

        // Handle Cost Sharing Document
        let costSharingDocUrl = edu.costSharingDocumentUrl;
        if (edu.costSharingDocument instanceof File) {
          try {
            costSharingDocUrl = yield call(
              uploadFileService,
        ...
        educationPayload.push({
          ...
          cost_sharing_document_url: costSharingDocUrl,
      ...
      localUpdate.educations = educationPayload.map((e: any) => ({
        ...e,
        institution: {
          name: typeof e.institution === "string" ? e.institution : e.institution?.name,
          category: e.institution_category || e.institutionCategory || e.institution?.category,
        },
        fieldOfStudy: {
          name: typeof e.field_of_study === "string" ? e.field_of_study : e.fieldOfStudy?.name,
        },
        educationLevel: {
          name: typeof e.level === "string" ? e.level : e.educationLevel || e.level,
        },
        costSharings: e.has_cost_sharing ? [{
          ...
          document_url: e.cost_sharing_document_url,
          currency: e.currency,
      ...
      promises.push(
        employeeService.updateEducation(id, educationPayload),
      );
    }
