...
import { FiUploadCloud, FiX, FiImage, FiArrowLeft } from "react-icons/fi";
...
export const EditCompany = () => {
  ...
  useEffect(() => {
    const fetchCompanyData = async () => {
      ...
      try {
        setFetching(true);
        const res = await platformService.getCompany(id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.companyName || !formData.companyCode) {
      toast.error("Please fill required fields (Name & Code)");
      return;
    }

    try {
      setLoading(true);
      await platformService.updateCompany(id, formData);
                ...
                {formData.logoUrl ? (
                  <>
                    ...
                    <button
                      type="button"
                      onClick={removeLogo}
                      className="absolute top-1 right-1 p-1 bg-red-500 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity underline-none"
                    >
