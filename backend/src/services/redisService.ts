...
class RedisService {
  ...
  private init() {
    ...
    try {
      console.log(
        `[Redis] Initializing client... URL: ${redisUrl ? "Yes" : "No"}, Host: ${redisHost}, Port: ${redisPort}`,
      );
      ...
      const commonOptions = {
        ...
        retryStrategy: (times: number) => {
          if (times > RedisService.MAX_RETRIES) {
            console.warn(
              `[Redis] Failed to connect after ${RedisService.MAX_RETRIES} attempts. Running without Redis caching.`,
            );
            this.hasGivenUp = true;
            return null; // Stop retrying
          }
          console.log(
            `[Redis] Retry attempt ${times}/${RedisService.MAX_RETRIES}...`,
          );
      ...
      this.client.on("error", (error) => {
        ...
        // Fail fast when Redis requires auth but password is not configured.
        if (message.includes("NOAUTH")) {
          this.hasGivenUp = true;
          console.warn(
            "[Redis] Authentication required but REDIS_PASSWORD is missing/invalid. Disabling Redis cache.",
          );
          try {
      ...
      this.client.on("end", () => {
        if (this.hasGivenUp) {
          console.warn(
            "[Redis] Client disconnected after max retries. Service will operate without caching.",
          );
        }
        this.client = null;
        this.isConnected = false;
  ...
  async delByPattern(pattern: string): Promise<void> {
    if (!this.client || !this.isConnected) {
      console.warn(
        `[Redis] Cannot delByPattern ${pattern}: Client not initialized`,
      );
      return;
    }
    try {
      ...
      console.log(
        `[Redis] delByPattern: Deleted ${totalDeleted} keys for pattern "${pattern}"`,
      );
    } catch (error) {
      console.error(
        `Error deleting keys with pattern ${pattern} from Redis:`,
        error,
      );
    }
  }
